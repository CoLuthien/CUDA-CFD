...
contains
    subroutine calculate_ghost_points(self)
        class(FCGrid3D), intent(inout) :: self
    ...
    impure function init_fcgrid3d(x, y, z, resolution, n_spc, cond) result(self)
        real(real64), allocatable, dimension(:, :, :), intent(inout) :: x, y, z
